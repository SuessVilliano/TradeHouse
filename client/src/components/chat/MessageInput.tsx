...
interface MessageInputProps {
  onSend: (content: string, replyTo?: string, signalData?: SignalData) => void;
    ...
    <div className="flex-shrink-0 px-4 pb-4 pt-2">
      ...
      {replyTo && (
        <div className="flex items-center gap-2 bg-th-border/50 rounded-t-lg px-3 py-2 border-l-2 border-th-accent mb-0.5">
          <div className="flex-1 min-w-0">
            <span className="text-[10px] text-th-accent font-semibold">
              Replying to {replyTo.members?.username || 'user'}
            </span>
            <p className="text-[10px] text-th-muted truncate">{replyTo.content}</p>
          </div>
          <button onClick={onCancelReply} className="text-th-muted hover:text-th-text transition-colors">
            <X size={14} />
          </button>
        </div>
      )}

      {/* Signal composer overlay */}
      {showSignal && (
        <SignalComposer
          onSend={(signal) => { onSend(`📊 Signal: ${signal.ticker}`, replyTo?.id, signal); setShowSignal(false); setText(''); }}
      ...
      <div className={`flex items-end gap-2 bg-th-input-bg border border-th-border rounded-xl px-3 py-2 focus-within:border-th-accent/50 transition-colors ${replyTo ? 'rounded-tl-none rounded-tr-none' : ''}`}>
        ...
        <button
          ...
          className={`mb-1 flex-shrink-0 transition-colors ${showSignal ? 'text-th-accent' : 'text-th-muted hover:text-th-text'}`}
          title="Post a trade signal"
        >
          <TrendingUp size={18} />
        </button>
        ...
        <div className="relative mb-1 flex-shrink-0">
          <button
            ...
            className={`transition-colors ${showEmoji ? 'text-th-accent' : 'text-th-muted hover:text-th-text'}`}
          >
            <Smile size={18} />
          </button>
          {showEmoji && (
  ...
  return (
    <div className="bg-th-sidebar border border-th-border rounded-xl p-4 mb-2 shadow-xl">
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm font-semibold text-th-text flex items-center gap-2">
          <TrendingUp size={14} className="text-th-accent" /> Post Trade Signal
        </span>
      ...
      <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-2">
        <input
          ...
          className="col-span-2 bg-th-input-bg border border-th-border rounded-lg px-3 py-1.5 text-th-text placeholder-th-muted text-xs focus:outline-none focus:border-th-accent"
          onChange={e => setSignal(prev => ({ ...prev, ticker: e.target.value.toUpperCase() }))}
          required
        />
        <div className="col-span-2 flex gap-2">
          {(['LONG', 'SHORT'] as const).map(dir => (
            <button
              key={dir}
              type="button"
              onClick={() => setSignal(prev => ({ ...prev, direction: dir }))}
              className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-colors ${
                signal.direction === dir
                  ? dir === 'LONG' ? 'bg-th-green text-black' : 'bg-th-red text-white'
                  : 'bg-th-border text-th-muted'
              }`}
            >
              {dir === 'LONG' ? '📈' : '📉'} {dir}
            </button>
        ...
        {[['Entry', 'entry'], ['Stop Loss', 'stopLoss'], ['Take Profit', 'takeProfit']].map(([label, key]) => (
          <input
            ...
            onChange={e => setSignal(prev => ({ ...prev, [key]: parseFloat(e.target.value) }))}
            required
          />
        ))}
        <input
          ...
          className="bg-th-input-bg border border-th-border rounded-lg px-3 py-1.5 text-th-text placeholder-th-muted text-xs focus:outline-none focus:border-th-accent"
          onChange={e => setSignal(prev => ({ ...prev, timeframe: e.target.value }))}
        />
        <textarea
          placeholder="Rationale (optional)"
          className="col-span-2 bg-th-input-bg border border-th-border rounded-lg px-3 py-1.5 text-th-text placeholder-th-muted text-xs focus:outline-none focus:border-th-accent resize-none"
          rows={2}
          onChange={e => setSignal(prev => ({ ...prev, rationale: e.target.value }))}
        />
        <button
          type="submit"
          className="col-span-2 bg-th-accent hover:bg-th-accent-hover text-white text-xs font-semibold py-2 rounded-lg transition-colors"
        >
          Post Signal
        </button>
      </form>
