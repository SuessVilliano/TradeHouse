...
interface LiveStreamProps { channel: Channel; user: AuthUser; }
...
export default function LiveStream({ channel, user }: LiveStreamProps) {
  ...
  const { room, error, connect, disconnect, isConnected, isConnecting } = useLiveKit({
    roomName, participantIdentity: user.id, participantName: username, canPublish: mode === 'broadcast',
  });

  const handleJoinWatch = async () => { setMode('watch'); await connect(); };
  const handleSetupBroadcast = async () => {
    setMode('broadcast'); setLoadingIngress(true);
    try { const result = await createOBSIngress(roomName); setIngressInfo({ rtmpUrl: result.rtmpUrl, streamKey: result.streamKey }); }
    catch (err) { console.error('Failed to create ingress:', err); }
    ...
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-th-chat gap-6">
        ...
        <div className="flex gap-4">
          <button onClick={handleJoinWatch} className="flex flex-col items-center gap-3 bg-th-sidebar border border-th-border hover:border-th-accent/50 hover:bg-th-accent/10 rounded-2xl p-6 w-44 transition-all group">
          ...
          <button onClick={handleSetupBroadcast} className="flex flex-col items-center gap-3 bg-th-sidebar border border-th-border hover:border-th-red/50 hover:bg-th-red/10 rounded-2xl p-6 w-44 transition-all group">
        ...
        <div className="flex-1 flex flex-col items-center justify-center gap-6 p-8">
          ...
          <div className="bg-th-border/40 rounded-xl px-4 py-3 max-w-md text-xs text-th-muted">
            ...
            <ol className="list-decimal list-inside space-y-1">
              <li>Open OBS → Settings → Stream</li><li>Select “Custom” service</li>
              <li>Paste the RTMP URL and Stream Key above</li><li>Click “Start Streaming” in OBS</li>
            </ol>
          </div>
          <button onClick={() => setMode('choose')} className="text-th-muted hover:text-th-text text-sm transition-colors">← Back</button>
        </div>
  ...
  if (!isConnected) {
    return (
      <div className="flex-1 flex items-center justify-center bg-th-chat">
        {error ? <div className="flex items-center gap-2 text-th-red"><AlertCircle size={16} /><span>{error}</span></div>
          : <div className="flex items-center gap-3 text-th-muted"><Loader2 size={20} className="animate-spin" /> Joining stream…</div>}
      </div>
    );
  }
      ...
      <div className="flex-1 flex bg-th-chat overflow-hidden">
        <div className="flex-1 flex flex-col min-w-0">
          ...
          <div className="border-t border-th-border px-4 py-3 flex items-center gap-3">
            ...
            <button onClick={disconnect} className="ml-auto text-sm text-th-muted hover:text-th-red transition-colors">Leave</button>
          </div>
        </div>
        <div className="w-80 border-l border-th-border flex flex-col">
          <div className="h-12 border-b border-th-border flex items-center px-4">
...
function StreamStage() {
  const tracks = useTracks([{ source: Track.Source.ScreenShare, withPlaceholder: false }, { source: Track.Source.Camera, withPlaceholder: false }]);
  const mainTrack = tracks.find(t => t.source === Track.Source.ScreenShare) || tracks.find(t => t.source === Track.Source.Camera);
  if (!mainTrack) {
    return (
  ...
  return <div className="flex-1 relative bg-black"><VideoTrack trackRef={mainTrack} className="w-full h-full object-contain" /></div>;
}
  ...
  return (
    <div className="bg-th-input-bg border border-th-border rounded-xl p-3">
      ...
      <div className="flex items-center gap-2">
        <span className="flex-1 text-xs text-th-text font-mono truncate">{secret ? '•'.repeat(Math.min(value.length, 24)) : value}</span>
        <button onClick={onCopy} className={`flex items-center gap-1 text-xs px-2 py-1 rounded-lg transition-colors ${copied ? 'text-th-green bg-th-green/10' : 'text-th-muted hover:text-th-text hover:bg-th-border'}`}>
